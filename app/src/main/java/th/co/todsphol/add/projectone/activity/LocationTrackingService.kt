...
open class LocationTrackingService : Service() {
    ...
    companion object {
        ...
        fun updateCameraBearing(googleMap: GoogleMap, bearing: Float) {
            if (false)
                return
            val ca = CameraPosition.builder(googleMap.cameraPosition).bearing(bearing).build()
            googleMap.animateCamera(CameraUpdateFactory.newCameraPosition(ca))
        }
    }
